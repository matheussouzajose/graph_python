...
async def list_companies(
    service: ServiceDep, limit: int = 100, offset: int = 0
) -> list[CompanyResponse]:
    companies = await service.list(limit=limit, offset=offset)
    return [CompanyResponse.model_validate(company) for company in companies]


@router.get("/{company_id}", response_model=CompanyResponse)
async def get_company(company_id: UUID, service: ServiceDep) -> CompanyResponse:
...
async def update_company(
    company_id: UUID, data: CompanyUpdate, service: ServiceDep
) -> CompanyResponse:
    try:
...
async def delete_company(company_id: UUID, service: ServiceDep) -> None:
